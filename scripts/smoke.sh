...
# S053-C3: the smoke harness drives authenticated mutations and receives a
# real session cookie. Refuse to point it at a non-loopback host unless the
# operator explicitly opts in, so a mistyped BASE cannot fire credentials or
# write traffic at a production (or otherwise remote) deployment.
base_hostport="${BASE#*://}"         # strip scheme (no-op when absent)
base_hostport="${base_hostport%%/*}" # strip any path suffix
case "$base_hostport" in
  \[*\]*)
    # Bracketed IPv6 literal, e.g. http://[::1]:3100 — drop brackets+port.
...
case "$base_host" in
  127.0.0.1|::1|localhost)
    # Curl normally inherits proxy variables and reads a user curl config.
    # For the safe-by-default loopback path, disable both so a local BASE
    # cannot be silently rerouted while carrying credentials or mutations.
    # --disable must be curl's first argument to suppress curlrc loading.
    CURL_POLICY_ARGS=(--disable --noproxy '*' --proxy '')
    ;;
  *)
    if [ "${SMOKE_ALLOW_REMOTE_BASE:-false}" != "true" ]; then
      echo "ERROR: smoke.sh refuses non-loopback BASE host '$base_host'." >&2
      echo "       The harness sends credentials and performs authenticated mutations." >&2
      echo "       Set SMOKE_ALLOW_REMOTE_BASE=true to opt in explicitly." >&2
      exit 1
    fi
    ;;
esac
...
csrf_token_for() {
  local jar="$1"
  smoke_curl -sS "${CURL_TLS_ARGS[@]}" -b "$jar" -c "$jar" -o /dev/null "$BASE/api/auth/me"
...
mutation_request() {
  ...
  body_file="$(mktemp)"
  MUTATION_STATUS="$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" -b "$jar" -c "$jar" -o "$body_file" -w '%{http_code}' \
    -X "$method" "$BASE$path" \
...
readiness_code=$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" -o /dev/null -w '%{http_code}' "$BASE/api/health/ready")
readiness_body=$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" "$BASE/api/health/ready")
...
if [ "$AUTH_DISABLED" != "true" ]; then
  code=$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" -o /dev/null -w '%{http_code}' "$BASE/login")
  check "login page renders" test "$code" = "200"
  # S053-C2: keep the password out of the process list. python3 reads the
  # secrets from its environment (never argv) and streams the login JSON to
  # curl on stdin, so `ps` shows neither the password nor the JSON body.
  SMOKE_EMAIL="$SMOKE_EMAIL" SMOKE_PASSWORD="$SMOKE_PASSWORD" python3 -c \
    'import json, os, sys; json.dump({"email": os.environ["SMOKE_EMAIL"], "password": os.environ["SMOKE_PASSWORD"]}, sys.stdout)' \
    | smoke_curl -sS "${CURL_TLS_ARGS[@]}" -c "$cookie_jar" -o /dev/null -X POST "$BASE/api/auth/login" \
      -H "Content-Type: application/json" \
      -H "Origin: $BASE_ORIGIN" \
      --data @-
  me=$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" -b "$cookie_jar" "$BASE/api/auth/me")
...
echo
echo "Product routes"
overview=$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" -b "$cookie_jar" "$BASE/dashboard/overview?year=2026")
...
data_entry=$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" -b "$cookie_jar" "$BASE/data-entry?year=2026")
...
reports=$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" -b "$cookie_jar" "$BASE/reports?view=board&year=2026")
...
trends=$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" -b "$cookie_jar" "$BASE/reports?view=trends&year=2026")
...
for path in \
  ...
  /dashboard/trends; do
  code=$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" -b "$cookie_jar" -o /dev/null -w '%{http_code}' "$BASE$path")
  check "$path is removed" test "$code" = "404"
done
for path in /api/entries /api/breakdowns /api/goals; do
  code=$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" -b "$cookie_jar" -o /dev/null -w '%{http_code}' -X POST "$BASE$path")
...
report_json=$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" -b "$cookie_jar" "$BASE/api/strategy/export?year=$year&format=json")
check "report export reads strategic calculation truth" grep -q '"report"' <<< "$report_json"
report_csv=$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" -b "$cookie_jar" "$BASE/api/strategy/export?year=$year&format=csv")
...
activity=$(smoke_curl -sS "${CURL_TLS_ARGS[@]}" -b "$cookie_jar" "$BASE/setup?area=activity")
