...
export function BoardReportView({ data }: { data: BoardReportPageData }) {
  ...
  const planSlug = data.report.plan?.lifecycleState === "archived"
    ? `-${data.report.plan.slug}`
    : "";
  return (
    <>
      <div className="mb-5 flex flex-wrap justify-end gap-2 no-print">
        ...
        <PrintButton />
        <ExportPNGButton targetId={targetId} fileName={`${data.report.organizationSlug}${planSlug}-board-report-${data.report.selectedYear}.png`} />
        <ExportPDFButton targetId={targetId} fileName={`${data.report.organizationSlug}${planSlug}-board-report-${data.report.selectedYear}.pdf`} />
      </div>
      <StrategicBoardReport
        id={targetId}
        report={data.report}
        boardScopeReviewStatus={data.boardScopeReviewStatus}
      />
    </>
  );
}
