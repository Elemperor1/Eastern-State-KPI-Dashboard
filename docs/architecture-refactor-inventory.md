...
- Forty-fifth protected slice: added a serial Playwright/Chrome acceptance suite for goal create/edit/goal-bearing PNG/delete, monthly save failure/retry/clear, desktop and mobile navigation, conversion/no-data PNG, category and metric legacy PDF, and native print PDF. The suite found a real `?legacy=1` hydration mismatch; category/metric server pages now parse that flag and pass it as an explicit client prop. All four browser workflows pass without unexpected console errors and clean up their temporary database rows. Final proof also includes 615 Vitest tests, lint/architecture/security guards, a production build, 56/56 real bypass smoke assertions, and 60/60 auth-enabled production smoke assertions.
...
| Catalog metadata | `src/features/catalog/server.ts`, `src/features/catalog/admin-catalog.ts`, `src/app/admin/kpis/*`, `src/components/AdminKpiCreateForm.tsx`, `src/components/AdminKpisTable.tsx`, `src/components/AdminCategoryCreateForm.tsx`, `src/components/AdminCategoriesList.tsx`, `src/app/api/categories/route.ts`, `src/app/api/kpis/route.ts` | Category and KPI ordering, lookup, create/update/delete operations, dependent-entry conflict checks, admin catalog filters/counts/options, create-form payload shaping, and guarded deletion confirmation models are feature-owned. Delete operations still refuse live monthly/breakdown dependents so entry audit tombstones are recorded before metadata removal. Catalog routes are mutation adapters only and return refreshed catalog lists for the browser admin client; catalog admin presentation is split into focused form/table/list components. |
| Metrics and entries | `src/features/metrics/*`, `src/app/admin/data/*`, `src/components/AdminDataFilters.tsx`, `src/components/AdminAnnualEntryEditor.tsx`, `src/components/AdminMonthlyEntryEditor.tsx`, `src/components/AdminBreakdownEntryEditor.tsx`, `src/app/api/entries/route.ts`, `src/app/api/breakdowns/route.ts`, `src/lib/analytics.ts` | Period rules, monthly/breakdown entry data access, available-year lookup, row mapping, admin page-data assembly, admin data-entry selection/draft construction/state transitions, breakdown edit-month resolution, saved-entry deletion identity, saved-breakdown update identity, mutation response validation, and entry audit writes are feature-owned. The `/admin/data` page is now an auth/render adapter; `AdminDataClient` owns browser form state, confirmation state, and guarded mutation calls. Entry/breakdown routes are mutation-only adapters with distinct `{ entry }` and `{ breakdown }` success payloads. |
...
- Updated `scripts/smoke.sh` to assert the finalized 52-KPI / 8-category catalog through `/admin/kpis`; `scripts/smoke-catalog.ts` supplies only local mutation IDs, and the D8AD-CAN-008 fake-server gate runs that helper in fixture mode.
