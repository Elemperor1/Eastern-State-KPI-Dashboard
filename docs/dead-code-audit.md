...
| `public/favicon.ico`, Galano fonts, mark logo, and `starfield.svg` | Metadata, `@font-face`, preload, `BrandMark`, login/setup styles | Build and browser workflows | High retain | retain as active; license is privately held and repository-owner attested |
| `public/fonts/LICENSE.txt` | No runtime import; records ownership and the private-license boundary without exposing the confidential document | Operational/legal support | High retain | retain as operational support |
